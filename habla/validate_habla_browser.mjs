...
async function waitTargets(){for(let i=0;i<160;i++){try{const r=await fetch(`http://127.0.0.1:${port}/json`);if(r.ok){const a=await r.json();if(Array.isArray(a)&&a.length)return a}}catch{}await sleep(100)}throw new Error('DevTools no respondió')}
...
try{
 ...
 for(const [W,H,label] of [[1280,800,'tablet'],[960,540,'compacta'],[854,393,'phone'],[800,360,'phone']]){
  ...
  const expr=`(async()=>{const f=[];const tick=(ms=0)=>new Promise(r=>setTimeout(r,ms));const inside=e=>{if(!e){f.push('elemento inexistente');return}const r=e.getBoundingClientRect(),s=getComputedStyle(e);if(s.display==='none'||s.visibility==='hidden')f.push('elemento oculto');if(r.left<-1||r.top<-1||r.right>innerWidth+1||r.bottom>innerHeight+1)f.push('elemento fuera viewport')};
   ...
   if(tiles.length===8){const hs=tiles.map(x=>x.getBoundingClientRect().height),ws=tiles.map(x=>x.getBoundingClientRect().width);if(Math.max(...hs)-Math.min(...hs)>2||Math.max(...ws)-Math.min(...ws)>2)f.push('familias desparejas')}
   for(const k of Object.keys(CATS)){openCategory(k);const c=CATS[k],opts=[...document.querySelectorAll('.option')],expected=Math.min(4,c.opts.length);if(opts.length!==expected)f.push(k+': cantidad visible incorrecta');inside(document.querySelector('.top'));inside(document.querySelector('.starter'));inside(document.querySelector('.options'));opts.forEach(inside);choosePhrase(Number(opts[0].dataset.idx||0),opts[0]);await tick();await tick();const idx=Number(opts[0].dataset.idx||0),x=c.opts[idx],p=document.getElementById('phrase');if(p.classList.contains('hidden'))f.push(k+': contenido no aparece');if(!p.textContent.includes(x.fEn)||!p.textContent.includes(x.fEs))f.push(k+': falta EN/ES');inside(p);const turn=document.getElementById('turn');if(turn.classList.contains('hidden'))f.push(k+': no invita a hablar');inside(turn);inside(document.getElementById('micStatus'));if(c.opts.length>4){const nav=document.getElementById('pageNav');if(nav.classList.contains('hidden'))f.push(k+': paginación oculta');inside(nav)}goHome()}
   openCategory('months');let mopts=[...document.querySelectorAll('.option')];if(document.getElementById('pageInfo').textContent!=='1 / 3')f.push('meses: página inicial incorrecta');choosePhrase(0,mopts[0]);await tick();await tick();if(currentKeyword!=='january')f.push('meses: palabra clave January no preparada');sessionVisits={0:1,1:1,2:1,3:1};currentOpt=3;advancePhrase();await tick();await tick();if(optionPage!==1)f.push('meses: no avanzó a página 2');const first2=document.querySelector('.option');if(!first2||first2.dataset.idx!=='4')f.push('meses: página 2 no empieza en May');goHome();
   openCategory('seasons');let sopts=[...document.querySelectorAll('.option')];choosePhrase(0,sopts[0]);await tick();await tick();if(currentKeyword!=='spring')f.push('estaciones: palabra clave Spring no preparada');goHome();
  ...
  const rr=await cdp('Runtime.evaluate',{expression:expr,returnByValue:true,awaitPromise:true});if(rr.exceptionDetails)throw new Error(rr.exceptionDetails.text||'Excepción JS');const v=rr.result.value;if(v.f.length)throw new Error(`${W}x${H} ${label}: ${v.f.join('; ')}`);console.log(`OK ${W}x${H} ${label}: 8 familias + meses/estaciones + EN/ES + palabra clave + dos intentos sin loop`)
 }
