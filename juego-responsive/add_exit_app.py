...
h=h.replace('<button class="bigbtn" onclick="startMemory()">¿CUÁL FALTA?</button><div class="small">Mirá, recordá y descubrí qué imagen desapareció.</div></section>',
            '<button class="bigbtn" onclick="startMemory()">¿CUÁL FALTA?</button><div class="small">Mirá, recordá y descubrí qué imagen desapareció.</div><button class="exitapp" onclick="closeApp()">✕ CERRAR APP</button></section>',1)
...
closefn="function closeApp(){try{cancelPairAudio()}catch(e){}try{if(window.AndroidVoice&&AndroidVoice.exitApp){AndroidVoice.exitApp();return}}catch(e){}window.close()}\n"
