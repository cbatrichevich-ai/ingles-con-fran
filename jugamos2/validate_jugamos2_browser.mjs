...
try{
 ...
 for(const [W,H,label] of [[1280,800,'tablet'],[1280,720,'tablet'],[960,540,'compacta'],[854,393,'phone'],[800,360,'phone']]){
  await cdp('Emulation.setDeviceMetricsOverride',{width:W,height:H,deviceScaleFactor:1,mobile:false});await cdp('Page.setDocumentContent',{frameId,html});await sleep(80);
  await cdp('Runtime.evaluate',{expression:`nativePair=(en,es,epoch)=>Promise.resolve(epoch===audioEpoch);browserPair=(en,es,epoch)=>Promise.resolve(epoch===audioEpoch);`});
  const expr=`(async()=>{const f=[];const tick=()=>new Promise(r=>setTimeout(r,0));const inside=e=>{if(!e){f.push('elemento inexistente');return}const r=e.getBoundingClientRect(),s=getComputedStyle(e);if(s.display==='none'||s.visibility==='hidden')f.push('elemento oculto');if(r.left<-1||r.top<-1||r.right>innerWidth+1||r.bottom>innerHeight+1)f.push('elemento fuera viewport')};
   ...
   startBingo();await tick();await tick();clearTimers();if(bingoBoard.length!==9)f.push('bingo no tiene 9');if(!bingoTarget)f.push('bingo no generó target tras audio inicial');if(bingoTarget){const txt=$('prompt').textContent;if(!txt.includes('Find the '+bingoTarget.w)||!txt.includes('Buscá '+bingoTarget.a))f.push('Bingo no muestra EN+ES');if(!lastPair||lastPair[0]!=='Find the '+bingoTarget.w||lastPair[1]!=='Buscá '+bingoTarget.a)f.push('Bingo no guarda audio EN+ES');const targetBtn=[...document.querySelectorAll('.bingo-cell')].find(b=>b.dataset.word===bingoTarget.w);if(!targetBtn)f.push('target Bingo no está en tablero');inside(document.querySelector('.top'));inside(document.querySelector('.bingo'));inside($('prompt'));[...document.querySelectorAll('.bingo-cell')].forEach(inside);const before=bingoMarked.size;tapBingo(targetBtn,bingoTarget);await tick();await tick();if(bingoMarked.size!==before+1)f.push('Bingo no marca acierto')}exitToHome();if($('home').classList.contains('hidden'))f.push('salir Bingo falla');

   startSequence();clearTimers();if(!seqItems.length)f.push('secuencia vacía');showSequenceStep(0);await tick();const seq=seqItems[0];if(seq){const txt=$('prompt').textContent;if(!txt.includes(seq.w)||!txt.includes(seq.s))f.push('Secuencia no muestra palabra EN+ES');if(!lastPair||lastPair[0]!==seq.w||lastPair[1]!==seq.s)f.push('Secuencia no pronuncia palabra EN+ES')}clearTimers();showSequenceAnswer();await tick();const turn=$('prompt').textContent;if(!turn.includes('Your turn')||!turn.includes('Tu turno'))f.push('Secuencia no muestra Tu turno bilingüe');const opts=[...document.querySelectorAll('.sequence-choice')];if(opts.length<seqItems.length)f.push('opciones secuencia insuficientes');seqItems.forEach(x=>{if(!opts.some(b=>b.dataset.word===x.w))f.push('falta item secuencia')});inside(document.querySelector('.sequence-options'));inside($('prompt'));opts.forEach(inside);const expected=seqItems[0],sb=opts.find(b=>b.dataset.word===expected.w);tapSequence(sb,expected);if(seqPos!==1)f.push('secuencia no avanza');exitToHome();

   startCatch();clearTimers();if(catchItems.length!==6)f.push('Atrápalo no tiene 6');if(catchTarget){const txt=$('prompt').textContent;if(!txt.includes('Catch the '+catchTarget.w)||!txt.includes('Atrapá '+catchTarget.a))f.push('Atrápalo no muestra EN+ES');if(!lastPair||lastPair[0]!=='Catch the '+catchTarget.w||lastPair[1]!=='Atrapá '+catchTarget.a)f.push('Atrápalo no guarda audio EN+ES')}const cb=[...document.querySelectorAll('.mover')].find(b=>b.dataset.word===catchTarget.w);if(!cb)f.push('target Atrápalo no aparece');inside(document.querySelector('.arena'));inside($('prompt'));[...document.querySelectorAll('.mover')].forEach(inside);const cr=catchRound;tapCatch(cb,catchTarget);await tick();await tick();if(catchRound!==cr+1)f.push('Atrápalo no avanza');exitToHome();
   return {f,w:innerWidth,h:innerHeight}})()`;
  const rr=await cdp('Runtime.evaluate',{expression:expr,returnByValue:true,awaitPromise:true});if(rr.exceptionDetails)throw new Error(rr.exceptionDetails.text||'Excepción JS');const v=rr.result.value;if(v.f.length)throw new Error(`${W}x${H} ${label}: ${v.f.join('; ')}`);console.log(`OK ${W}x${H} ${label}: portada + 3 juegos + consignas EN/ES + viewport`)
 }
 console.log('VALIDACIÓN JUGAMOS 2 RUNTIME BILINGÜE OK');
