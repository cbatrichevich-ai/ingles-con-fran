...
public class MainActivity extends Activity {
 ...
 private void begin(String e){expected=e;diag("1 · Pedido recibido por Android");if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED){diag("2 · Falta permiso de micrófono");requestPermissions(new String[]{Manifest.permission.RECORD_AUDIO},REQ_MIC);return;}diag("2 · Permiso concedido");startRecognizer();}
 private void startRecognizer(){destroyRecognizer();diag("3 · Comprobando reconocimiento");if(!SpeechRecognizer.isRecognitionAvailable(this)){err(100,"No hay servicio de reconocimiento disponible.");return;}diag("4 · Servicio disponible");try{if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.S&&SpeechRecognizer.isOnDeviceRecognitionAvailable(this)){diag("5 · Creando reconocedor local");recognizer=SpeechRecognizer.createOnDeviceSpeechRecognizer(this);}else{diag("5 · Creando reconocedor del dispositivo");recognizer=SpeechRecognizer.createSpeechRecognizer(this);}}catch(Exception e){diag("5B · Fallback a reconocedor normal");try{recognizer=SpeechRecognizer.createSpeechRecognizer(this);}catch(Exception ex){err(102,"No se pudo crear: "+ex.getClass().getSimpleName());return;}}
...
print('LAB diagnóstico aplicado sobre base congelada: sólo RED; 187 preservado.')
