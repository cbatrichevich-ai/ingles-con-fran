...
public class MainActivity extends Activity {
 ...
 private void begin(String e){expected=e;retriedNormal=false;diag("1 · Pedido recibido por Android");if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED){diag("2 · Falta permiso de micrófono");requestPermissions(new String[]{Manifest.permission.RECORD_AUDIO},REQ_MIC);return;}diag("2 · Permiso concedido");startRecognizer(true);}
 private void startRecognizer(boolean preferLocal){destroyRecognizer();diag("3 · Comprobando reconocimiento");if(!SpeechRecognizer.isRecognitionAvailable(this)){err(100,"No hay servicio de reconocimiento disponible.");return;}diag("4 · Servicio disponible");usingLocal=false;try{if(preferLocal && Build.VERSION.SDK_INT>=Build.VERSION_CODES.S&&SpeechRecognizer.isOnDeviceRecognitionAvailable(this)){diag("5 · Creando reconocedor local");recognizer=SpeechRecognizer.createOnDeviceSpeechRecognizer(this);usingLocal=true;}else{diag("5 · Creando reconocedor normal");recognizer=SpeechRecognizer.createSpeechRecognizer(this);}}catch(Exception e){diag("5B · Fallback a reconocedor normal");try{recognizer=SpeechRecognizer.createSpeechRecognizer(this);usingLocal=false;}catch(Exception ex){err(102,"No se pudo crear: "+ex.getClass().getSimpleName());return;}}
 recognizer.setRecognitionListener(new RecognitionListener(){public void onReadyForSpeech(Bundle b){diag("7 · LISTO: Android está escuchando");js("window.onAndroidMicReady&&window.onAndroidMicReady()");}public void onBeginningOfSpeech(){diag("8 · Voz detectada");}public void onRmsChanged(float f){}public void onBufferReceived(byte[] b){}public void onEndOfSpeech(){diag("9 · Fin de voz detectado");}public void onError(int e){if(e==SpeechRecognizer.ERROR_LANGUAGE_UNAVAILABLE && usingLocal && !retriedNormal){retriedNormal=true;diag("13 · Inglés no descargado para reconocimiento local. Probando reconocedor normal...");startRecognizer(false);return;}String m=(e==SpeechRecognizer.ERROR_LANGUAGE_NOT_SUPPORTED)?"El reconocedor actual no admite inglés.":(e==SpeechRecognizer.ERROR_LANGUAGE_UNAVAILABLE)?"El inglés no está disponible actualmente en este reconocedor.":(e==SpeechRecognizer.ERROR_NO_MATCH)?"No alcancé a entender.":(e==SpeechRecognizer.ERROR_SPEECH_TIMEOUT)?"No escuché tu voz.":(e==SpeechRecognizer.ERROR_NETWORK||e==SpeechRecognizer.ERROR_NETWORK_TIMEOUT)?"El reconocimiento necesita conexión en este dispositivo.":"Código Android "+e;err(e,m);destroyRecognizer();}public void onResults(Bundle b){ArrayList<String> r=b.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION);String h=(r!=null&&!r.isEmpty())?r.get(0):"";diag("10 · Resultado: "+h);js("window.onAndroidSpeechResult&&window.onAndroidSpeechResult("+JSONObject.quote(h)+","+JSONObject.quote(expected)+")");destroyRecognizer();}public void onPartialResults(Bundle b){}public void onEvent(int e,Bundle b){}});
 ...
 @Override public void onRequestPermissionsResult(int r,String[] p,int[] g){super.onRequestPermissionsResult(r,p,g);if(r==REQ_MIC){if(g.length>0&&g[0]==PackageManager.PERMISSION_GRANTED){diag("2A · Permiso concedido. Tocá RED otra vez.");js("window.onAndroidPermissionGranted&&window.onAndroidPermissionGranted()");}else err(101,"Permiso rechazado.");}}
...
async function startRedMic(label){micCue(false);micDiag('Audio de indicación...');await playFile('prompt_ahora_vos.wav');micCue(true,false);micDiag('Esperando para abrir reconocimiento...');document.getElementById('status').textContent='🎙️ Tu turno: '+label;await new Promise(r=>setTimeout(r,1200));if(window.AndroidSpeech&&AndroidSpeech.startListening){micDiag('Enviando pedido a Android...');AndroidSpeech.startListening(label)}else{micDiag('ERROR · No existe puente AndroidSpeech')}}
...
if oldfun not in h: raise SystemExit('No se encontró playContent de la base congelada; STOP técnico')
...
print('LAB RED: fallback automático de local a normal si falta modelo inglés; 187 preservado.')
