...
newfun="""let micPermissionReady=false;
...
function closeEnough(candidate,expected){const c=normSpeech(candidate),w=normSpeech(expected);if(!c||!w)return false;if(c===w)return true;if(w==='RED'&&['READ','BREAD'].includes(c))return true;if(c.length<=w.length+1&&editDistance(c,w)<=1)return true;return false}
function onAndroidPermissionReady(){micPermissionReady=true;micDiag('')}
function requestListen(label,delay=500){micCue(true,false);document.getElementById('status').textContent='🎙️ Tu turno: '+label;setTimeout(()=>{if(window.AndroidSpeech&&AndroidSpeech.startListening)AndroidSpeech.startListening(label)},delay)}
...
function onAndroidSpeechError(code,message){micCue(true,false);document.getElementById('status').textContent='👂 Probemos otra vez';micDiag(message);requestListen('RED',1000)}
async function playContent(file,label,askRepeat=true){micCue(false);micDiag('');document.getElementById('status').textContent='👂 Escuchá: '+label;try{await playFile(file);if(askRepeat){await new Promise(r=>setTimeout(r,350));if(file==='001.wav'){requestListen(label,500)}else{await playFile('187.wav');document.getElementById('status').textContent='🗣️ Ahora vos: '+label}}}catch(e){}}"""
...
print('LAB RED: permiso al iniciar app; reconocedor normal en inglés; escucha automática tras RED; tolerancia pedagógica; prompt ruidoso fuera del circuito; resto de la app intacto.')
